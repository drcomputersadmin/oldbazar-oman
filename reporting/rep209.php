...
function print_po()
{
	...
	$cols = array(4, 60, 280, 340, 400, 450, 505, 600);

	// $headers in doctext.inc
	$aligns = array('left',	'left',	'center', 'right', 'right', 'right', 'right');
		...
		foreach($tax_items as $tax_item)
		{
			...
			if ($myrow['tax_included'])
			{
				if ($SysPrefs->alternative_tax_include_on_docs() == 1)
				{
					if ($first)
					{
						$rep->TextCol(2, 6, _("Total Tax Excluded"), -2);

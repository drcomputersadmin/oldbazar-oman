...
function print_sales_orders()
{
	...
	for ($i = $from; $i <= $to; $i++)
	{
		...
		while ($myrow2=db_fetch($result))
		{
		 ...
		 $NetWithtax = $Net + $tax_amount;
			...
			if ($Net != 0.0 || !is_service($myrow2['mb_flag']) || !$SysPrefs->no_zero_lines_amount())
			{
				...
				$rep->TextCol(6, 7,	"  ".$Net, -2);
				$rep->TextCol(7, 8,	"  ".$tax_rate_percentage_display, -2);
				$rep->TextCol(8, 9,	"  ".$tax_amount, -2);
		...
		$rep->NewLine(0.1);
		$words = price_in_words2($myrow["freight_cost"] + $SubTotal, ST_SALESORDER);
		if ($words != "")
		{
			$rep->NewLine(1);
			$rep->TextCol(0, 6, "TOTAL PAID IN WORDS: " . $words . " " , - 2);
			$rep->TextCol(0, 4, "_______________________",-2);
		}	
		$rep->NewLine(1.8);

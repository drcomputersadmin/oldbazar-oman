...
$rep->NewLine(-7);
			...
			// }
			if (strtoupper($myrow['curr_code']) == 'OMR') {
    $words = price_in_words2($myrow['Total'], ST_CUSTPAYMENT);
} else {
    $words = price_in_words($myrow['Total'], ST_CUSTPAYMENT);
}

// Output the result if $words is not empty
if ($words != "") {
    $rep->NewLine(1);
    $rep->TextCol(0, 7, $myrow['curr_code'] . ": " . $words, -2);
}
			$rep->NewLine(2);
			$rep->TextCol(5, 6, _("Total Allocated"), -2);
			$rep->AmountCol(6, 7, $total_allocated, $dec, -2);
			$rep->NewLine(1.5);
			$rep->TextCol(5, 6, _("Left to Allocate"), -2);
			$rep->AmountCol(6, 7, $myrow['Total'] + $myrow['ov_discount'] - $total_allocated, $dec, -2);
			if (floatcmp($myrow['ov_discount'], 0))
			{
				$rep->NewLine(1.5);
				$rep->TextCol(5, 6, _("Discount"), - 2);
				$rep->AmountCol(6, 7, -$myrow['ov_discount'], $dec, -2);
			}	
			$rep->NewLine(1.5);
			$rep->Font('bold');
			$rep->TextCol(5, 6, _("TOTAL RECEIPT"), - 2);

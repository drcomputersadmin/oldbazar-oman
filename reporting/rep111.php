...
function print_sales_quotations()
{
	...
	$cols = array(4, 20, 70, 330, 355, 385, 417, 453, 485, 510);

	// $headers in doctext.inc
	// $aligns = array('left',	'left',	'right', 'center', 'right', 'right', 'right');
	$aligns = array('center', 'left', 'left', 'center', 'center', 'right', 'right', 'center', 'right', 'right');
	...
	for ($i = $from; $i <= $to; $i++) {
		...
		while ($myrow2 = db_fetch($result)) {
			...
			$tax_rate = get_tax_type_rate($myrow2['tax_type_id']);
			if ($tax_rate !== false) {
				// Calculate tax amount based on the tax rate and net amount
				$tax_rate_percentage = $tax_rate / 100;
				$tax_amount = round2(($tax_rate / 100) * $Net, user_price_dec());

				// Other code...
			} else {
				// Handle case where tax rate is not found
				// You can log an error or handle it based on your application's requirements
				// For example:
				echo "Error: Tax rate not found for tax type ID " . $myrow2['tax_type_id'];
			}



			//added new for multiple tax in quot
			...
			if ($tax_rate_percentage == 0) {
				...
				$tax_amountnew = round2(($tax_rate_percentage / 100) * $Net, user_price_dec());
			}



			$NetWithtax = $Net + $tax_amountnew;
			$prices[] = $Net;
			...
			$rep->TextCol(0, 1, $serial_number++, -2);
			$rep->TextCol(1, 2, "  " . $myrow2['stk_code'], -2);
			$oldrow = $rep->row;
			// $rep->TextColLines(1, 2, $myrow2['description'], -2);
			$rep->TextColLines(2, 3, $myrow2['concatenated_description'], -2);
			...
			$newrow = $rep->row;
			// $tax_rate_percentage_display = number_format($tax_rate_percentage * 100, 2) . '%';
			...
			if ($Net != 0.0 || !is_service($myrow2['mb_flag']) || !$SysPrefs->no_zero_lines_amount()) {
				...
				$rep->TextCol(7, 8,	"  " . $tax_rate_percentage_display, -2);
				$rep->TextCol(8, 9, "  " . number_format2($tax_amountnew, $dec), -2);
		...
		$rep->NewLine(0.1);
		$words = price_in_words2($myrow["freight_cost"] + $SubTotal, ST_SALESQUOTE);
		if ($words != "") {
			$rep->NewLine(1);
			$rep->Font('bold');
			$rep->TextCol(0, 4, "TOTAL PAID IN WORDS:  " . $words . " ", -2);
			// $rep->TextCol(0, 2, "____________________", -2);
		}
		...
		$rep->TextCol(9, 10,	$DisplayTotalDiscount, -2);
		$rep->NewLine(1.5);
		...
		if ($myrow['tax_included'] == 0) {
			$rep->TextCol(5, 7, _("TOTAL (EX VAT)"), -2);
			$rep->TextCol(9, 10,	$DisplayTotal, -2);
			$rep->NewLine(1.5);
		}

		$tax_items = get_tax_for_items(
			$items,
			$prices,
			$myrow["freight_cost"],
			$myrow['tax_group_id'],
			...
			if ($myrow['tax_included']) {
				if ($SysPrefs->alternative_tax_include_on_docs() == 1) {
					if ($first) {
						...
						$rep->TextCol(9, 10,	number_format2($tax_item['net_amount'], $dec), -2);
						$rep->NewLine();
					}
					$rep->NewLine(0.5);
					...
					$rep->TextCol(5, 7, _("Included") . " " . $tax_type_name, -2);
				$rep->TextCol(9, 10, $DisplayTax, -2); // Adjusted column indexes
		...
		$DisplayTotal = number_format2($myrow["freight_cost"] + $SubTotal, $dec);
		$rep->Font('bold');
		$rep->TextCol(5, 7, _("TOTAL( VAT INCL)"), -2);
		$rep->TextCol(9, 10,	$DisplayTotal, -2);

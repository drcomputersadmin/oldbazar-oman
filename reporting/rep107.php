...
function print_invoices()
{
	...
	$cols = array(4, 20, 70, 330, 355, 385, 420, 455, 485, 515);
	...
	while ($row = db_fetch($range)) {
		...
		while ($myrow2 = db_fetch($result)) {
			...
			$NetWithtax = $Net + $TaxAmount;
			$NetRounded = number_format2($Net, $dec);
			$tax_amountRounded = number_format2($TaxAmount, $dec);
			$SubTotal += $Net;
			$DisplayPrice = number_format2($myrow2["unit_price"], $dec);
			$DisplayQty = number_format2($sign * $myrow2["quantity"], get_qty_dec($myrow2['stock_id']));

			if ($myrow['tax_included'] == 0) {
				$DisplayNet = number_format2($NetWithtax, $dec);
				}
				else{
					$DisplayNet = number_format2($Net, $dec);
				}
			// $DisplayNet = number_format2($NetWithtax, $dec);
			$NetWithoutDiscount += $myrow2["unit_price"] * $myrow2["quantity"];
			if ($myrow2["discount_percent"] == 0)
				$DisplayDiscount = "";
			else
				$DisplayDiscount = number_format2($myrow2["discount_percent"] * 100, user_percent_dec()) . "%";
			$totalDiscount += $myrow2["discount_percent"] * $myrow2["unit_price"] * $myrow2["quantity"];
			$c = 0;

			$rep->TextCol($c++, $c, $serial_number++, -2);
			$rep->TextCol($c++, $c,		" " . $myrow2['stock_id'], -2);
			$oldrow = $rep->row;
			$description = strtoupper($myrow2['StockDescription'] . ' - ' . $myrow2['StockLongDescription']);
			$tax_rate_percentage = $myrow2['TaxRate'];
			$tax_rate_percentage_display = $tax_rate_percentage == 0
				? '0%'
				: rtrim(number_format($tax_rate_percentage * 100, 2), ".0") . '%';


			// Your other printing logic...
			$rep->TextColLines($c++, $c, $description, -2);
			...
			if ($Net != 0.0 || !is_service($myrow2['mb_flag']) || !$SysPrefs->no_zero_lines_amount()) {
				...
				$rep->TextCol($c++, $c,	"  " . $NetRounded, -2);
				$rep->TextCol($c++, $c,		"  " . $tax_rate_percentage_display, -2);
				$rep->TextCol($c++, $c,		"  " . $tax_amountRounded, -2);
		...
		$rep->NewLine(0.1);
		$words = price_in_words2(
			$rep->formData['prepaid'] ? $myrow['prep_amount'] : $myrow['Total'],
		...
		if ($words != "") {
			...
			$rep->TextCol(0, 4, "TOTAL PAID IN WORDS: " . $words . " ", -2);
			// $rep->TextCol(0, 2, "___________________",-2);

		}
		$DisplayTotalDiscount = number_format2($totalDiscount, $dec);
		$DisplayNetWithoutDiscount = number_format2($NetWithoutDiscount, $dec);
		$label_start_col = 4;  // Starting column for labels
		$label_end_col = 6;    // Ending column for labels
		$amount_start_col = 8; // Starting column for amounts
		$amount_end_col = 10;  // Ending column for amounts

		$rep->aligns[$amount_start_col] = 'right';
		$rep->row = $summary_start_row;
		$rep->cols[2] += 20;
		$rep->cols[3] += 20;
		$rep->aligns[3] = 'left';
		$rep->NewLine(3);
		$rep->TextCol($label_start_col, $label_end_col, _("SUB TOTAL     "), -2);
		$rep->TextCol($amount_start_col, $amount_end_col, $DisplayNetWithoutDiscount, -2);
		$rep->NewLine(1.5);
		$rep->TextCol($label_start_col, $label_end_col, _("DISCOUNT      "), -2);
		$rep->TextCol($amount_start_col, $amount_end_col, $DisplayTotalDiscount, -2);
		$rep->NewLine(1.5);
		...
		while ($tax_item = db_fetch($tax_items)) {
			...
			if ($myrow['tax_included']) {
				if ($SysPrefs->alternative_tax_include_on_docs() == 1) {
					if ($first) {
						$rep->TextCol($label_start_col, $label_end_col, _("Total Tax Excluded"), -2);
						$rep->TextCol(8, 10, number_format2($sign * $tax_item['net_amount'], $dec), -2);
						$rep->NewLine();
					}
					$rep->TextCol($label_start_col, $label_end_col, $tax_type_name  , -2);
					$rep->TextCol(8, 10, $DisplayTax, -2);
					$first = false;
				} else
				$rep->TextCol($label_start_col, $label_end_col, _("Included"). " " .$tax_type_name . "        ", -2);
				$rep->TextCol(8, 10, $DisplayTax, -2);
		...
		$rep->TextCol($label_start_col, $label_end_col, $rep->formData['prepaid'] ? _("TOTAL ORDER VAT INCL.") : _("TOTAL INVOICE"), -2);
		$rep->TextCol(8, 10, $DisplayTotal, -2);
		if ($rep->formData['prepaid']) {
			$rep->NewLine(2);
			$rep->Font('bold');
			$rep->TextCol($label_start_col, $label_end_col, $rep->formData['prepaid'] == 'final' ? _("THIS INVOICE") : _("TOTAL INVOICE"), -2);
